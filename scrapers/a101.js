...
async function getCampaignApiBase(campaignLink) {
    if (campaignLink.includes('/liste/')) {
        return { 
            promotionCode: null, fieldType: null, totalItems: 0, 
            error: "Bu kampanya linki API yerine doğrudan HTML ile ürün listesi gösteriyor." 
        };
    }

    let promotionCode = null;
    let fieldType = 'promotionCode'; // Varsayılan
    ...
    if (!promotionCode) {
        // Linkin sonundaki S5842, Z100 gibi yapıları yakalar
        const linkCodeMatch = campaignLink.match(/-([SZC])([0-9]{3,5})$/i);
        if (linkCodeMatch) {
            const letter = linkCodeMatch[1].toUpperCase();
            const numbers = linkCodeMatch[2];

            if (letter === 'S') {
                fieldType = 'collectionId'; // Senin verdiğin çalışan URL'lerdeki yapı
                promotionCode = numbers;    // 'S5842' yerine sadece '5842'
            } else {
                fieldType = 'promotionCode';
                promotionCode = letter + numbers; // 'Z100' gibi
            }
        }
    }

    return { promotionCode: promotionCode, fieldType: fieldType, totalItems: 0, error: promotionCode ? null : "Kod bulunamadı" };
}



async function generateAllApiUrls(promotionCode, fieldType) {
    const apiUrls = [];
    let currentFrom = 0;
    const MAX_PAGES = 7; 

    while (currentFrom < (MAX_PAGES * ITEMS_PER_PAGE)) {
        const apiPayload = {
            channel: "SLOT",
            filters: [{ 
                field: fieldType, // 'collectionId' veya 'promotionCode'
    ...
    while (true) {
        ...
        try {
            ...
            $products.each((index, el) => {
                ...
                const imageUrl = $imageElement.attr('src') || $imageElement.attr('data-src') || $imageElement.attr('data-origin');
        ...
        for (const campaign of sideCampaigns) {
            ...
            if (campaign.link.includes('/liste/')) {
                ...
                if (apiBaseResult.error) {
                    ...
                    // main() fonksiyonu içinde bul ve bu satırla değiştir:
                    const apiUrls = await generateAllApiUrls(apiBaseResult.promotionCode, apiBaseResult.fieldType);
